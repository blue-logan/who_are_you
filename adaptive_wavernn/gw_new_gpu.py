...
def calc_transfer(area, r_start, r_parallel, r_series, z_power, z_val, targets,\
        w_id, upper):
    ...
    for r_id in range(r_start, r_start + r_parallel):
        ...
        if r_id < r_series.size:            #check edge case
            ...
            #area[r_id][w_id] = abs(float64(targets[w_id]) - 20 *\
            #        log10(abs(number_final / denom_final_0)))
            area_tmp = abs(float64(targets[w_id]) - 20 *\
                    log10(abs(number_final /denom_final_0)))
