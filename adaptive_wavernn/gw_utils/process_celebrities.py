...
def goto_datadir():
	...
	os.chdir(config['celebrity_datadir'])
...
def master_sentences():
	...
	for line in open('sentences.txt','r').readlines():
		if line != '\n':
			line = line.strip().replace('\xe2\x80\x99','\'')
			master_sentences_dict[sentence_id] = line
			sentence_id+=1
		# 	print(line.strip())
		# number, sentence = line.split(')')
		# master_sentences_dict[int(number)] = sentence[1:].strip()
	pprint(dict(master_sentences_dict))
	return dict(master_sentences_dict)
...
def voice_join(df_formants, df_acoustic):
    #Join frames
    new_df = pd.merge(df_formants, df_acoustic,  how='left', on=['id','sentence_number','name'])
...
if __name__ == '__main__':
	...
	# Load physical table
	physical_df = pd.read_csv('celebs.csv')
	#makes a dictionary that maps {id: name}

	celebrity_dict = {person_id: name for _, (person_id,name) in physical_df[['id','file']].iterrows()}
	pprint(celebrity_dict)
	# sys.exit()

	# grabs all subjects in the root data directory
	# A subject is the directory where all the data is stored for *one* individual
	# e.g., root/person1/
	subjects = [subject for subject in os.listdir(config['celebrity_datadir']) if 'person' in subject]
	...
	for idx, subject in enumerate(subjects, start=1):
		...
		for audio_filename in sorted(sentence_filenames()):
			...
			#add name column
			df_p['name'] = celebrity_dict[id_number]
			df_f['name'] = celebrity_dict[id_number]
